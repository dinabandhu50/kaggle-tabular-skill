...
- `src/device.py` (GPU auto-detect), `src/tracking.py` (optional wandb logging, off unless `WANDB_PROJECT` is set — the ledger is the source of truth regardless), `src/optimize/optuna_search.py` (reusable, config-driven Phase-5 tuning), `src/feature_engineering/` (starts empty — agents fill it with numbered, per-model FE implementations; see the skill's `references/feature-engineering.md`), `PROGRESS.md` (append-only rollup journal updated after every experiment/submission).
...
- **Template files are placeholders, not live code.** Use `{{COMP_NAME}}` for the competition slug; never hardcode a competition. GPU is auto-detected at runtime by `src/device.py::has_gpu()` and used by default in the GBDT wrappers — no flag needed. The `--gpu` scaffold flag *pins* the device instead (e.g. `device_type="gpu" if has_gpu() else "cpu",` → `device_type="gpu",  # pinned by --gpu scaffold flag`) — preserve that exact line form so `enable_gpu()` in the scaffolder can find and swap it.
